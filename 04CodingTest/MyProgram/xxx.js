(function() {
	var c = {
		id: "e3c7f7f17054135e8f86f4c943459d8b",
		dm: ["oneplus.cn"],
		etrk: [],
	...
	function da(a, b, d) {
		...
		try {
			window.localStorage ? (b = e.getTime() + "|" + b, window.localStorage.setItem(a, b)) : D() && (C.expires = e.toUTCString(), C.load(document.location.hostname), C.setAttribute(a, b), C.save(document.location.hostname))
		} catch(f) {}
	}
	function ea(a) {
		if (window.localStorage) {
			...
			return C.load(document.location.hostname),
			C.getAttribute(a)
		} catch(e) {}
		return n
	};
	...
	var ga = /msie (\d+\.\d+)/i.test(navigator.userAgent),
	ha = navigator.cookieEnabled,
	ia = navigator.javaEnabled(),
	ja = navigator.language || navigator.browserLanguage || navigator.systemLanguage || navigator.userLanguage || "",
	pa = window.screen.width + "x" + window.screen.height,
	qa = window.screen.colorDepth;
					...
					if (H(a, "Object")) {
						var k = function(a) {
							for (var b in a) if ({}.hasOwnProperty.call(a, b)) {
								var d = a[b];
								H(d, "Object") || H(d, "Array") ? k(d) : a[b] = String(d)
							}
						};
						k(a);
						this.a.api |= 16;
	...
	function wa() {
		var a = v("Hm_unsent_" + c.id);
		if (a) for (var a = a.split(","), b = 0, d = a.length; b < d; b++) w(L + "//" + decodeURIComponent(a[b]).replace(/^https?:\/\//, ""),
		function(a) {
			T(a)
		})
	}
	function T(a) {
		var b = v("Hm_unsent_" + c.id) || "";
	...
	function sa(a) {
		try {
			...
			a: if (document.referrer) {
				for (var m = 0,
				ya = c.se.length; m < ya; m++) if (RegExp("(^|\\.)" + c.se[m][1].replace(/\./g, "\\.")).test(t(document.referrer))) {
					var B = s(document.referrer, c.se[m][2]) || "";
				...
				B = p;
				U(document.referrer) && U(document.location.href) ? B = l: (K = t(document.referrer), B = V(K || "", document.location.hostname));
			...
			0 == a.a.nv ? wa() : T(".*");
			if ("" != c.icon) {
				var y, z = c.icon.split("|"),
				$ = "http://tongji.baidu.com/hm-web/welcome/ico?s=" + c.id,
				aa = ("http:" == L ? "http://eiv": "https://bs") + ".baidu.com" + z[0] + "." + z[1];
				switch (z[1]) {
				case "swf":
					var la = z[2],
					ma = z[3],
					N = "s=" + $,
					x = "HolmesIcon" + J;
					y = '<object classid="clsid:d27cdb6e-ae6d-11cf-96b8-444553540000" id="' + x + '" width="' + la + '" height="' + ma + '"><param name="movie" value="' + aa + '" /><param name="flashvars" value="' + (N || "") + '" /><param name="allowscriptaccess" value="always" /><embed type="application/x-shockwave-flash" name="' + x + '" width="' + la + '" height="' + ma + '" src="' + aa + '" flashvars="' + (N || "") + '" allowscriptaccess="always" /></object>';
					break;
				case "gif":
					y = '<a href="' + $ + '" target="_blank"><img border="0" src="' + aa + '" width="' + z[2] + '" height="' + z[3] + '"></a>';
					break;
				default:
					y = '<a href="' + $ + '" target="_blank">' + z[0] + "</a>"
				}
				document.write(y)
			}
			...
			if (O && Ga.test(O) && Ha) {
				...
				P.setAttribute("src", L + "//" + c.js + s(O, "jn") + "." + s(O, "sx") + "?" + a.a.rnd);
				var na = document.getElementsByTagName("script")[0];
				na.parentNode.insertBefore(P, na)
			}
			a.l && a.l();
			a.k && a.k();
			if (c.rec || c.trust) a.a.nv ? (a.c = encodeURIComponent(document.referrer), window.sessionStorage ? u("Hm_from_" + c.id, a.c) : da("Hm_from_" + c.id, a.c, 864E5)) : a.c = (window.sessionStorage ? v("Hm_from_" + c.id) : ea("Hm_from_" + c.id)) || "";
			a.m && a.m();
			a.n && a.n();
			a.g = new Ba;
			F(window, "beforeunload", Aa(a));
			var A = window._hmt;
			...
			if ("undefined" === typeof window._bdhm_account || window._bdhm_account === c.id) {
				...
				var Q = window._hmt;
				if (Q && Q.length) for (var A = 0,
				Ja = Q.length; A < Ja; A++) a.p(Q[A]);
				window._hmt = a.q
			}
			...
			w(L + "//hm.baidu.com/hm.gif?" + a.join("&"))
		}
	}
	function za(a) {
	...
	function va() {
		...
		try {
			if (ca(a, "", {
				...
				C.load(document.location.hostname),
				C.removeAttribute(a),
				C.save(document.location.hostname)
			} catch(b) {}
		} catch(d) {}
	}
	function ua() {
	...
	R.prototype.k = function() {
		var a = this;
		c.ctrk && (F(document, "mouseup", Ma(this)), F(window, "beforeunload",
		function() {
			W(a)
	...
	function Ma(a) {
		return function(b) {
			var d, e;
			ga ? (e = Math.max(document.documentElement.scrollTop, document.body.scrollTop), d = Math.max(document.documentElement.scrollLeft, document.body.scrollLeft), d = b.clientX + d, e = b.clientY + e) : (d = b.pageX, e = b.pageY);
	...
	R.prototype.m = function() {
		var a = this;
		c.rec && fa(function() {
			var b = document.getElementById("hm_t_" + c.rp);
			if (c.rp && c.rs && !(2 == c.rs && !b || b && "" != b.innerHTML)) {
	...
	function Ba() {
		...
		this.j = 0;
		"object" == typeof document.onfocusin ? (F(document, "focusin", Y(this)), F(document, "focusout", Y(this))) : (F(window, "focus", Y(this)), F(window, "blur", Y(this)))
	}
	function Y(a) {
		return function(b) {
